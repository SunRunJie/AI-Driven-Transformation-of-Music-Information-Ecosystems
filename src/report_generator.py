...
from config import (
    FIGURES_DIR, ANALYSIS_FIGURES_DIR, FILES, REPORT_META, REFERENCES,
...
def _insert_figure(f, filename: str, caption: str):
    ...
    path = ANALYSIS_FIGURES_DIR / filename
    if path.exists():
        kb = path.stat().st_size // 1024
        rel = f"../figures/analysis/{filename}"
        f.write(f"![{caption}]({rel})\n*Figure: {caption} ({kb}KB)*\n\n")
    else:
