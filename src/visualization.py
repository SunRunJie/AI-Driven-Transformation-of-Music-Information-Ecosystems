...
from config import (
    FIGURES_DIR, CHATGPT_RELEASE_DATE, RANDOM_SEED,
...
def plot_ai_feature_comparison(
    ...
    gs = GridSpec(1, 3, figure=fig, width_ratios=[1.2, 0.05, 1.3], wspace=0.2)
    ...
    # Feature name labels (placed around the outside of the radar, aligned with the ticks)
    ...
    for ang, col in zip(label_angles, compare_cols):
        ...
        # Place the label in the outer ring at the polar tick position
        ax1.text(ang, 1.25, label, fontsize=9, fontweight="bold",
                ha="center", va="center", color="#333333",
                rotation=angle_deg if 90 < angle_deg < 270 else 0,
...
def plot_competitive_landscape(save: bool = True) -> plt.Figure:
    ...
    for _, row in high_risk.iterrows():
        ax.annotate("[WARN] High risk",
                    (row["data_depth"] + 0.1, row["social_engagement"] - 0.8),
...
def generate_all_figures(
    ...
    for g in generated:
        if hasattr(g, 'name'):
            size_kb = (FIGURES_DIR / g.name).stat().st_size // 1024
